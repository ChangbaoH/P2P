...
import com.hcb.p2p.base.mapper.MailVerifyMapper;
import com.hcb.p2p.base.mapper.UserinfoMapper;
import com.hcb.p2p.base.service.IMailService;
	...
	public void bindPhone(String phoneNumber, String verifyCode) {
		...
		if (!current.getIsBindPhone()) {
			...
			if (ret) {
				...
				//谨记此处需要乐观锁，只能调用update而不能调用mapper的更新
				this.update(current);
			} else {
				// 抛出异常;
